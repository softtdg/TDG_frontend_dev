          ...
          <div>
            <FormButton
              ...
              onClick={() => {
                ...
                const encodedDate = FinalDeliveryDate
                  ? btoa(FinalDeliveryDate)
                  : "";
      ...
      formatter: (_: any, row: any) => {
        function formatDeptComments(entries: any[]) {
          ...
          for (const entry of entries) {
            ...
            // Department Comments
            if (
              entry.LeadHandCommentsForPurchasing ||
              entry.PurchasingComments
            ) {
            ...
            if (entry.LeadHandCommentsForDesign || entry.DesignComments) {
              ...
              result.push({
                Dept: "Design",
                Notice: `${date || "N/A"}: ${
                  entry.LeadHandCommentsForDesign || ""
                }`.trim(),
            ...
            if (entry.LeadHandCommentsForSales || entry.SalesComments) {
              ...
              result.push({
                Dept: "Sales",
                Notice: `${date || "N/A"}: ${
                  entry.LeadHandCommentsForSales || ""
                }`.trim(),
            ...
            if (Array.isArray(entry.backorderEntry)) {
              entry.backorderEntry.forEach((backorder: any) => {
                ...
                const notice = closedDate
                  ? `Missing: ${backorder.TDGPN} (${backorder.Quantity?.toFixed(
                      2
                    )}) ${backorder.UOM} - CLOSED - ${closedDate}`
                  : `Missing: ${backorder.TDGPN} (${backorder.Quantity?.toFixed(
                      2
                    )}) ${backorder.UOM}`;
              ...
              {(() => {
                return (
                  <div className="p-2 w-full h-full">
                    <div className="">
                      {(() => {
                        const deptComments = formatDeptComments(
                          data2?.filter(
                            (item: any) =>
                              item?.SOPLeadHandEntryId ===
                              row?.SOPLeadHandEntryId
                          )
                        );
                            ...
                            <tbody>
                              {deptComments.length > 0 ? (
                                deptComments.map((comment, index) => {
                                  const deptColorClass =
                                    comment.Dept === "Purchasing"
                                      ? "bg-[#607D99]"
                                      : comment.Dept === "Design"
                                      ? "bg-[#8A9B6B]"
                                      : "bg-[#E4E1FF]";
                                  return (
                                    ...
                                    >
                                      ...
                                      {comment.Response ? (
                                        ...
                                        <td className="border px-2 py-1 bg-[#913734] w-50 text-xs">
                                          {comment.Response}
                                        </td>
                                      )}
                                    </tr>
